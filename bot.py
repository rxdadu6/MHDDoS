...
if result:  
    ...
    if datetime.now() > expiration_date:  
        ...
        vip_status = (  
            f"✅ VIP CLIENT!\n"  
            f"⏳ Remaining days: {remaining_days} day(s)\n"  
...
bot.reply_to(  
    message,  
    (  
        "🤖 *WELCOME TO CRASH BOT [Free Fire]!*"  
...
def handle_addvip(message):  
    ...
    if len(args) != 3:  
        bot.reply_to(  
            message,  
            "❌ Invalid format. Use: `/vip <ID> <DAYS>`",  
            parse_mode="Markdown",  
        )  
        return  
    ...
    bot.reply_to(message, f"✅ User {telegram_id} added as VIP for {days} days.")  
...
def handle_stop_attack(call):  
    ...
    if telegram_id in active_attacks:  
        ...
        bot.answer_callback_query(call.id, "❌ No active attack found. Proceed with your action.")  
